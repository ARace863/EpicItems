...
public class StartupMethods {
    ...
    public void checkForUpdate() {
        new UpdateChecker(plugin, 93541).getVersion(version -> {
            if (plugin.getDescription().getVersion().equals(version)) {
                plugin.getLogger().info("You are running the latest version of EpicItems: " + plugin.getDescription().getVersion());
            } else {
                plugin.getLogger().info("There is a new update available. Download it here: https://www.spigotmc.org/resources/epic-items-custom-items.93541/");
            }
        });
    }
    ...
    public void registerEvents() {
        ...
        plugin.getServer().getPluginManager().registerEvents(new GUIMethods(plugin), plugin);
        ...
        plugin.getServer().getPluginManager().registerEvents(new VaultCheck(plugin), plugin);

        plugin.getServer().getPluginManager().registerEvents(new MoveQuiverEvent(), plugin);
    }
